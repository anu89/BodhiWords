...
export default function TestQuestion({ question, index, total, onAnswer }: Props) {
  ...
  const checkAnswer = (ans: string) => {
    const isCorrect = ans.trim().toLowerCase() === question.answer.trim().toLowerCase()
    return isCorrect
  }
  ...
  const handleFillSubmit = () => {
    ...
    setRevealed(true)
    const correct = checkAnswer(fillValue)
    setTimeout(() => onAnswer(fillValue, correct), 900)
  }
          ...
          {question.options && (
            <div className="space-y-2.5">
              {question.options.map(opt => {
                let variant: 'idle' | 'correct' | 'wrong' | 'missed' = 'idle'
                if (revealed) {
                ...
                return (
                  <motion.button
                    ...
                    className={cn(
                      ...
                      variant === 'wrong' && 'border-red-400 bg-red-50 text-red-700 wrong-shake',
                    )}
                  >
